...
if TYPE_CHECKING:
    from databricks.rag_eval.clients import (
        managed_evals_client,
        managed_rag_client,
        mlflow_client,
    )
...
class Context(ABC):
    ...
    def build_mlflow_client(self) -> "mlflow_client.MLFlowClient":
        """
        Build a MLflow client for the current eval session.
        """
        pass
...
class RealContext(Context):
    ...
    def build_mlflow_client(self) -> "mlflow_client.MLFlowClient":
        from databricks.rag_eval.clients import mlflow_client

        return mlflow_client.MLFlowClient()
